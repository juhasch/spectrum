...
def _build_tridiagonal(N: int, NW: float):
    ...
    This uses the formulation from Percival and Walden (1993) which matches
    _other_dpss_method and produces correct DPSS sequences:
      diag[i] = cos(2*pi*W) * (( (N-1)/2 - i )^2)
      offdiag[i] = i * (N - i) / 2 for i>=1
    
    Note: The original C code uses negative signs, but that formulation
    requires different eigenvalue selection. This positive-sign formulation
    matches the working _other_dpss_method implementation.
    """
    W = float(NW) / float(N)
    twopi = 2.0 * np.pi
    cs = np.cos(twopi * W)
...
def dpss_tapers(N: int, NW: float, Kmax: int):
    ...
    # Select the Kmax eigenvectors with the largest eigenvalues
    # This matches _other_dpss_method which uses select_range=(N-Kmax, N-1)
    w, v = np.linalg.eigh(T)
    idx = np.argsort(w)[-Kmax:]
    # Ensure ascending order across the selected set
    idx.sort()
    evecs = v[:, idx]
    # Reverse order to match eig_banded's behavior (eig_banded returns in descending order)
    evecs = evecs[:, ::-1]
